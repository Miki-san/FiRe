...
int smoke[numOfData];
int CO_1[numOfData / 2];
int CO_2[numOfData / 2];
int CO2[numOfData];
int temp[numOfData];
int smokeIterator = 0;
int COIterator1 = 0;
...
void getDHTTemp() {
  ...
  switch (dht.getState()) {
    case DHT_OK:
      temp[TempIterator] = dht.getTemperatureC();
...
void sensorThreadSetup() {
  ...
  TempdataThread.onRun(getDHTTemp);
  ...
  if (!client.connected()) {
    ...
    if (status == WL_CONNECTED) {
      if (calibratingStatus) {
        if (sensorFlag && (smoke[numOfData - 1] != 0) && (CO_1[(numOfData / 2) - 1] != 0) && (CO_2[(numOfData / 2) - 1] != 0) && (CO2[numOfData - 1] != 0)) {
        ...
        avgSmoke /= (float)numOfData;
        avgCO /= (float)numOfData;
        avgCO2 /= (float)numOfData;
        avgTemp /= (float)numOfData;
        if (!sensorFlag) {
          dataPrint();
          dataSend();
        }
